    ...
    def __init__(self, name, opt, val):
        ...
        if (opt.upper() == 'PAR'):
            ...
            # parameters of the gamma distribution: a = k, scale = 1/lambda
                ...
                '''
                gumbel min is the left skewed gumbel implemented in scipy
                gumbel_l parameters: an = scale, bn = location
                '''
                if (val[1] > 0):
                    self.Par[0] = val[0]
                    self.Par[1] = val[1]
                    self.Dist = scipy.stats.gumbel_l(location=val[0],
                                                     scale=val[1])
                else:
                ...
                '''
                gumbel is the right skewed gumbel implemented in scipy gumbel_r
                parameters: an = scale, bn = location
                '''
                if (val[1] > 0):
                    self.Par[0] = val[0]
                    self.Par[1] = val[1]
                    self.Dist = scipy.stats.gumbel_r(loc=val[0],
                                                     scale=val[1])
                else:
                    raise RuntimeError("The Gumbel distribution is not defined"
                                       " for your parameters")
